...
public class Tree {
	...
	private int size(Node root) { 
		...
		return currNodeCount + size(root.lfChild) + size(root.midChild)+ size(root.rtChild);
	}
	...
	// In order for BST: left, visit, right. For 23 Tree I think it will be left, visit, mid, visit, right ?
	...
	private Integer get(Node root, int targetIndex, int currIndex) {
		...
		if(root.lfChild == null && root.midChild == null && root.rtChild == null) { // Are we in a leaf? BASE CASE
			...
			}else if(targetIndex == ++currIndex && root.getVal(1) != null) { // Does the 2nd val in leaf contain the index of item we look for?
		...
		if(targetIndex == currIndex) { // If we get here it's because we didn't find anything in left subtrees. Check if maybe this node has our val
			return root.getVal(0);
		}else if(targetIndex == ++currIndex) { // TODO: Will this line work? I will think about it when it's not 2:20 am
			return root.getVal(1);
		}
		
		get(root.midChild, targetIndex, ++currIndex); // Means we didnt find val in left subtree. Go back to where we started going down left subtree and 
													  // go down to midChild, then start going down left subtree again.
		...
		Integer leftValIndex = currIndex + size(root.lfChild); 
		Integer rightValIndex = null;

		if(root.getVal(1) != null) {//ie we have a right val in our node
			rightValIndex = leftValIndex + 1;
			if(root.midChild != null) { // ie we have a middle child.
				rightValIndex+= size(root.midChild);
			}
		}
		
		
		if(targetIndex < leftValIndex) {
			get(root.lfChild, targetIndex, 0); // Travelling left, no need to account for trees in right subtree
		}
	...
	private Node find(Node root, Integer target) { // Traverses tree until it finds a node w/ a matching value. If nothing is found, return null.
		...
		// Three cases. 1) Val is less than leftmost OR 2) Val is inbetween leftmost and rightmost OR 3) val is greater than rightmost OR
		// 4) rightval doesn't exist but target is greater than leftmost, threfore go down right subtree.
		
		
		// leftmost val is bigger than target ie case 1
		if(leftMost > target) {
			return find(root.lfChild, target);
		}
		
		// Target is inbetween leftmost and rightmost aka case 2
		if(target > leftMost && rightMost != null && target < rightMost) {
			return find(root.midChild, target);
		}
		
		
		// Target is greater than rightmost. case 3
		if(rightMost != null && target > rightMost) {
			return find(root.rtChild, target);
		}
		
		// node only contains one value
		if(rightMost == null && target > leftMost) {
			return find(root.rtChild, target);
		}
	...
	private void split(Node root) {
		...
		//	parent.children.set(0, new Node(root.getVal(0), parent));// sets left child of new parent
			//parent.children.set(1, new Node(root.getVal(2), parent)); // sets right child of new parent
		}
	}
	...
	private void insert(Node root, int val) { // I am essentially using DFS
		...
		// Is our val equal to leftmost val OR right most val if it exists? Then it exists and we don't add duplicates.
		if(root.getVal(0) == val || ((root.getVal(1) != null && root.getVal(1) == val))) { // BASE CASE #2
			return;
		}
		
		// We have reached a leaf. Therefore add. I don't care if there are 2 already in a node, split function will handle splitting.
		if(root.lfChild == null && root.rtChild == null && root.midChild == null) { // BASE CASE #3
			root.addVal(val);
			size++;
			split(root); // TODO: Find a way to split. Create method and call it here.
			return;
		}
	...
	private class Node{ 
		...
		public Integer[] vals; // Keeps track of values in a node (referred to as 'keys' in video). // 0 = leftmost, 1 = rightmost, etc
